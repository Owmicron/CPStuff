...
int n;
pair<ld,ld> enginex[110],enginey[110];
ld ans;
int main(){
...
//    freopen("output.txt", "w", stdout);
    ...
    sort(enginex+1,enginex+1+n);
    sort(enginey+1,enginey+1+n);

    ld x,y; x=0;y=0;
    for(int i=1;i<=n;i++){
        x+=enginex[i].fi;
        y+=enginex[i].se;
        ld dist = sqrt( ld(x) * ld(x) + ld(y) * ld(y));
        ans = max(dist,ans);
    }
    x=0;y=0;
    for(int i=n;i>=1;i--){
    ...
    cout<<fixed<<setprecision(30)<<ans<<newl;
}
