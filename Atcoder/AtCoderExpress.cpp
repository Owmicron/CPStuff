...
//    freopen("output.txt", "w", stdout);
    ...
    for(int i=1;i<=T;i++)vmax[i]=INT_MAX;
    T=0;
    for(int i=1;i<=n;i++){
        cin>>v[i];

        for(int j=T;j<=T+t[i];j++){
    ...
    //for(int i=1;i<=T;i++)cout<<i<<" "<<vmax[i]<<newl;
    for(int i=1;i<=T;i++){
    ...
    //cout<<newl;for(int i=1;i<=T;i++)cout<<i<<" "<<vmax[i]<<newl;
    for(int i=1;i<=T;i++){
        ans += ((vmax[i]+vmax[i-1])/4);
    }
    cout<<fixed<<setprecision(5)<<ans<<newl;
}
