...
int stree[200010 * 4],n;
multiset <int> s[200010];

int a[200010],b[200010];
void build(int now, int l, int r){
    if(l == r){
        stree[now] = (s[l].empty() ? INT_MAX : *s[l].rbegin());
        return;
    }
    int mid = (l + r) >> 1;
    build(now*2,l,mid);
...
void update(int now, int l, int r, int idx){
    ...
    int mid = (l + r) >> 1;
    update(now*2,l,mid,idx);
    update(now*2+1,mid+1,r,idx);
    stree[now] = min(stree[now*2],stree[now*2+1]);
    return;
}
int main(){
...
//    freopen("output.txt", "w", stdout);
    ...
    while(q--){
        ...
        s[b[idx]].erase(s[b[idx]].find(a[idx]));
        update(1,1,N,b[idx]);

        b[idx] = as;
        s[b[idx]].insert(a[idx]);
        update(1,1,N,b[idx]);
