...
bool lu(int r, int c){
    ...
    if(r>1 && c>1){
        int x = r-1;
        int y = c-1;
        int f = a[x][y] + a[x+1][y] + a[x][y+1] + a[x+1][y+1];
        if(f==4)res=true;
    }
    return res;
}
...
//    freopen("output.txt", "w", stdout);
    ...
    for(int i=1;i<=n;i++){
        for(int j=1;j<=m;j++){
            if(a[i][j]==1){
                if(lu(i,j)){ans.insert(mp(i-1,j-1));}
                else if(ru(i,j)){ans.insert(mp(i-1,j));}
                else if(ld(i,j)){ans.insert(mp(i,j-1));}
                else if(rd(i,j)){ans.insert(mp(i,j));}
