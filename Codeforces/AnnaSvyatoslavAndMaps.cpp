...
// end of Template
int INF = 1e7;
int n,m,p[1000010],dist[200][200],vis[200][200];
vi adj[200];
void shortest(int x){
    queue < pii > q;
    q.push(mp (x,0));
    ...
    while(!q.empty()){
        ...
        int step = q.front().se;
        //cout<<node<<" "<<step<<newl;
        dist[x][node] = min(dist[x][node],step);
        q.pop();
        if(!vis[x][node]){
            vis[x][node] = true;
...
//    freopen("output.txt", "w", stdout);
    ...
    for(int i=1;i<=n;i++){
        ...
        for(int j=0;j<s.size();j++){
            int id = j+1;
            if(s[j]=='1'){adj[i].pb(id);}
        }
    }
    cin>>m;
    for(int i=1;i<=m;i++)cin>>p[i];

    for(int i=1;i<=n;i++)shortest(i);
    //cout<<dist[1][4]<<newl;
    ...
    while(idx<m){
        ...
        int nex = 1;
        for(int i=1;i<=n;i++){
            //cout<<now<<" "<<i<<" "<<dist[now][i]<<newl;
            if(idx + dist[now][i] <= m && dist[now][i]>0){
                if(p[idx+dist[now][i]] == i){
                    if(dist[now][i] > nex){nex = dist[now][i];}
                }
            }
        }
        v.pb(p[idx]);
